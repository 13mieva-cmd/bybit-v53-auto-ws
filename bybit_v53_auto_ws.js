...
async function fetchKlines(symbol, interval, limit=120){
  const res = await get(`/v5/market/kline?category=spot&symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(String(interval))}&limit=${limit}`);
...
function tg(r){ return [`${r.status} — ${r.symbol}`,`score: ${r.score}/100`,`1h: ${r.bullish1h ? 'bullish' : 'bearish'}`,`15m: ${r.bullish15 ? 'confirm' : 'weak'}`,`news: ${r.news ? 'yes' : 'no'}`,`in-play: vol x${r.volSpike}, atr x${r.atrSpike}, spread ${r.spreadPct}%, ws ${r.wsMom}%`,`entry: ${r.entry}`,`sl: ${r.stop}`,`tp1: ${r.tp1}`,`tp2: ${r.tp2}`,`24h: ${r.change24h_pct}% | vol: ${Math.round(r.turnover24h_usdt)} | age: ${r.age_days}d`,`ann: ${ANN_URL}`].join('\n'); }
    ...
    try {
      ...
      if(sig.status === 'LONG'){
        ...
        if(state.lastAlert !== fp){
          state.lastAlert = fp;
          await postTelegram(tg(sig)).catch(e => log(`TG ERR ${e.message}`));
        }
      }
    } catch (e) {
...
function startHealthServer(){
  http.createServer((req, res) => {
    ...
    res.end('OK');
  }).listen(PORT, '0.0.0.0', () => log(`Health server on port ${PORT}`));
}
